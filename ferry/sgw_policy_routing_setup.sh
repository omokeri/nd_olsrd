...
function generic() {
  echo "$IPTABLES $IPTABLES_ARGS -t mangle $ADDMODE_IPTABLES PREROUTING  -m conntrack ! --ctstate NEW -j CONNMARK --restore-mark"
  echo "$IPTABLES $IPTABLES_ARGS -t mangle $ADDMODE_IPTABLES OUTPUT      -m conntrack ! --ctstate NEW -j CONNMARK --restore-mark"
}
...
function olsrif() {
  ...
  echo "$IP       $IP_ARGS        rule      $ADDMODE_IP iif    $interfaceName table main       priority $bypassRuleNr"
}
...
function egressif() {
  ...
  echo "$IPTABLES $IPTABLES_ARGS -t mangle $ADDMODE_IPTABLES POSTROUTING -m conntrack --ctstate NEW -o $interfaceName -j CONNMARK --set-mark $ruleNr"
  echo "$IPTABLES $IPTABLES_ARGS -t mangle $ADDMODE_IPTABLES INPUT       -m conntrack --ctstate NEW -i $interfaceName -j CONNMARK --set-mark $ruleNr"
  echo "$IP       $IP_ARGS        rule      $ADDMODE_IP fwmark $ruleNr        table $tableNr   priority $ruleNr"
  echo "$IP       $IP_ARGS        rule      $ADDMODE_IP iif    $interfaceName table main       priority $bypassRuleNr"
}
...
function sgwsrvtun() {
  ...
  echo "$IPTABLES $IPTABLES_ARGS -t mangle $ADDMODE_IPTABLES PREROUTING  -m conntrack --ctstate NEW -i $interfaceName -j CONNMARK --set-mark $ruleNr"
  echo "$IP       $IP_ARGS       rule      $ADDMODE_IP fwmark $ruleNr table $tableNr priority $ruleNr"
}
...
declare addMode="$4"
shift 4
declare logFile="$(dirname "$script")/$scriptName-$instanceId.log"
argc=$#
